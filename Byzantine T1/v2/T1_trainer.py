...
from keras import Sequential, layers, regularizers, optimizers, initializers
...
class relu3_Qagent_linearOut_dOut_l2():
    ...
    def create_model(layer1_size, layer2_size, layer3_size, layer4_size, 
                     dropout1, dropout2, dropout3, reg1, reg2, reg3, learning_rate, input_shape: tuple, output_size: int, seed):
        model = Sequential()
        model.add(layers.Input(shape=input_shape))
        model.add(layers.Dense(layer1_size, activation="relu", kernel_regularizer=regularizers.l2(reg1), kernel_initializer=initializers.HeNormal(seed=seed)))
        model.add(layers.Dropout(dropout1))
        model.add(layers.Dense(layer2_size, activation="relu", kernel_regularizer=regularizers.l2(reg2), kernel_initializer=initializers.HeNormal(seed=seed)))
        model.add(layers.Dropout(dropout2))
        model.add(layers.Dense(layer3_size, activation="relu", kernel_regularizer=regularizers.l2(reg3), kernel_initializer=initializers.HeNormal(seed=seed)))
